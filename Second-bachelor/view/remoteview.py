...
class RemoteView(View):
    def __init__(self, database: DatabaseControler, rules: RulesControler, player: int):
        super(View).__init__(database, rules, player)
        self._current_ip_address = self.get_IP()
        self._other_ip_address = None
    ...
    def user_input(self):
        if self._connection is None:
            raise ValueError("The connection is not init")
        return get_remote_orders(self._connection)
        
    
    def ask_IP_address(self, player: int):
        ...
        """
        other_IP = ''
        pattern = r"[0-9]{3}\.[0-9]{2}\.[0-9]{3}\.[0-9]{3}"
        while other_IP == '':
            try:
                other_IP = input(f'Enter the IP of the group {player} if this is not the arbitrator of the tournaments : ')
                if other_IP == '':
                    other_IP = '127.0.0.1' # two programm on same computer will met soon...
                elif re.search(pattern, other_IP) is None:
        ...
        self._other_ip_address = other_IP
    
    #  override
    def init_connection(self, my_group, player: int):
        print(f"Your IP is : {self._current_ip_address}")
        self.ask_IP_address(player)
        self._connection = create_connection(
            my_group,
            player, 
            other_IP=self._other_ip_address, 
            verbose=False
        )
    ...
    def send_order(self, order: str):
        if self._connection is None:
            raise ValueError("The connection is not init")
        notify_remote_orders(self._connection, order)
